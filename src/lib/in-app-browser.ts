 ...
 *
 * Detection order: Telegram JS bridges, then a UA token list. Missing
 * `win` (SSR) is false.
 *
 ...
 * iOS Telegram (`TelegramWebviewProxy` or UA `Telegram`, plus iPhone/iPad/iPod,
 * and `url` starting with `https://`) → `x-safari-` + url on `location.href`.
...
export function openInSystemBrowser(url: string, win?: SystemBrowserHost): void {
  ...
  const isTelegram = host.TelegramWebviewProxy !== undefined || /Telegram/i.test(ua);
  if (isIos && isTelegram && url.startsWith('https://')) {
