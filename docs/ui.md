...
| Aspect       | 1:1 only for the profile portrait. Forum message photos stay `max-h-80 w-full rounded-xl object-contain` for a single still. When `photoCount > 1`, stills sit in a horizontal snap scroller (`overflow-x-auto snap-x snap-mandatory`), each still in a `min-w-full shrink-0 snap-start` slide, not a vertical stack. |
...
3. Optional photo/video (`rounded-xl`, `max-h-80`). Multiple stills (`photoCount > 1`) sit in a horizontal snap scroller (`overflow-x-auto snap-x snap-mandatory`), each still in a `min-w-full shrink-0 snap-start` slide.
