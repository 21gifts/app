...
| Aspect       | 1:1 only for the profile portrait. Forum message photos stay `max-h-80 w-full rounded-xl object-contain` for a single still. When `photoCount > 1`, stills sit in `ForumPhotoGallery`: 88% snap slides so the next photo peeks, `current/total` chip, dots — not a full-width stack that hides further stills. |
...
3. Optional photo/video (`rounded-xl`, `max-h-80`). Multiple stills (`photoCount > 1`) use `ForumPhotoGallery` (88% peek of the next still, `current/total` chip, dots).
