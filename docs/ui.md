...
**Safe area / visualViewport.** `AppShell` plus `--app-height` (bootstrap script + `useAppHeight` / `AppHeightSync`) is the height source. When no text field is focused, `--app-height` uses `max(visualViewport.height, innerHeight)` so a stuck-short visual viewport cannot shrink the rounded page frame. While a text field is focused, `--app-height` follows `visualViewport.height` so the software keyboard does not cover the composer. Bootstrap uses the max path (first paint is unfocused). Pinch-zoom skip is unchanged: do not follow a visual viewport whose `scale` is present and not ≈ 1. `html { touch-action: manipulation }` disables double-tap-zoom; pinch-zoom stays. Do not add `env(safe-area-inset-*)` here.
