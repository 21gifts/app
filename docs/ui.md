...
Content translation **Translate** under a note or reply body is icon-only (`IconButton` + Languages, `aria-label` = `forum.translate`). It stays under the body, not in the footer icon row. **Show original** / **Show translation** stay labeled underline text.
