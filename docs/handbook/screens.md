...
- **What the user sees:** One **Menu** top-right; open it for Profile, language, and **Log out**. Gift icon, **Welcome, {name}**, public forum (message name, text, timestamp, sat total with a Bitcoin pay icon beside it when the note is payable, composer textarea with **Post** to the right). No name or address form. No guest donate CTA.
