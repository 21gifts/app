...
- **Purpose:** Signed-in member identity card (chart, About me inside the card — not a forum post, name, location, Lightning Address, role pill, copy-profile-link, and clickable post/reply counts from `postCount` / `replyCount`) with on-demand activity feeds below the card. Location is read-only. Own profiles use this route too (forum author names navigate here, not `/profile`). The Message button sits on the card (not on a post) when another member has a `profileMessage`. Forum posts/replies in those feeds keep labeled **Translate** / Show original / Show translation via `NoteTranslate` when the language differs from the UI locale (not in the footer icon row, and not on About me). A missing name, Lightning Address, or rules agreement on a reply opens `RequirementsOverlay` (no Skip). Signed-in chrome may show `IntroduceYourselfOverlay` when `setup` is null and `hasPosted` is false.
The in-card reply composer includes an **Amount** sats field; non-exempt visitors invoice at least 1 sat. - **Inputs:** Bearer session; `accountId` UUID; `GET /forum/members/:id` for the profile (`aboutMe`, read-only `location`) and activity counts; Given + Received from `GET /forum/members/:id/activity` (`fetchMemberActivity`). Fetch activity even when Lightning Address is blank; activity failure keeps the card with empty series. On-demand `GET /forum/members/:id/posts` or `GET /forum/members/:id/replies` for the selected feed.
- **Actions:** Open **Menu** for **Home**, Profile, **Living room rules**, **Notifications**, **Messages**, **Contact**, optional **Install app**, language, or **Log out**; icon-only back to the forum; expand role hint; copy the member profile link (`profile.copyLink` **Copy link to this profile** → origin `/members/:id`, URL not shown); Message on the card to open a private thread; click the post or reply count to open that `ForumBoard` feed below the card, or click the pressed count again to collapse it. There is no separately pinned profile-note `ForumBoard`. Translate a foreign-language post or reply in the feed (**Translate** / Show original / Show translation). Posts retain pay, expand, and reply actions. Reply cards are not payable, and expanding one with a `parentId` navigates to `/messages/{parentId}`. When a listed feed is shorter than its count, a muted `profile.activityLatest` truncation line shows the displayed and total counts. Complete a `RequirementsOverlay` for a missing name, Lightning Address, or rules agreement before a reply; dismiss `IntroduceYourselfOverlay` for this mount or follow **Write an introduction** to `/welcome`. Location is read-only. No edit controls.
...
Both series non-zero: received three UTC days totaling ₿1,500 and donated ₿2,100 on **2026-06-02**. Chart shows Given and Received together (`state-profile-given-received`).
...
- **Actions:** Click a row to open the public forum note `/messages/{parentId}` (mark that notification read). Back to the forum. Open **Menu** for **Home**, Profile, **Living room rules**, **Notifications**, **Messages**, **Contact**, optional **Install app**, language, or **Log out**. Dismiss `IntroduceYourselfOverlay` for this mount or follow **Write an introduction** to `/welcome`.
