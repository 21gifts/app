...
- **Purpose:** Format an instant as the UTC calendar day string `YYYY-MM-DD`.
- **Inputs:** Optional `now` (`Date`; default `new Date()`).
- **Returns / side effects:** UTC day string from `toISOString().slice(0, 10)`. No I/O.
- **Used by:** `ForumLoader` when fetching today’s outbound gifts.
...
## Function: ForumTodayGifts

- **Purpose:** One collapsed line summarizing today’s platform outbound gifts (`Today {count} gifts · {usd}`), with a labeled disclosure button (`aria-expanded`, expand/collapse `aria-label`s). Expanding lists each gift as `{recipient} · {usd}` (WoS handle as-is) and a link **All gifts this day** to `/stats/{day}`. No times, sats, charts, or `GiftDayTable` / `StatsDashboard`.
- **Inputs:** `ForumTodayGiftsProps` — required `day` (`GiftDay`).
- **Returns / side effects:** React client element. Local expand state only; formats USD via `formatUsdDisplay`. No network.
- **Used by:** `ForumBoard` when `todayGifts.giftCount > 0` and the composer is shown.
...
The No gifts yet mode keeps only loaded messages with exactly zero sats, including notes without a wallet, preserving input order. The board displays these oldest first. Active remains the default.
