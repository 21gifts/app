...
- **Purpose:** Renders the guest donate form (Lightning Address and sat amount only; no comment) and, after success, the invoice QR. All visitor-facing copy goes through `useTranslations`.
...
- **Purpose:** Next.js page for `/handbook`. Loads the four app handbook files at build time (`force-static`) and renders them with a link to the api handbook. Title and intro chrome are localized via `HandbookIntro`; markdown bodies stay English.
