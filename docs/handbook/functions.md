...
- **Purpose:** Third post-login screen: one living-room rules chapter at a time and an **I agree** control. Intermediate agrees only advance the chapter. The last agree POSTs and merges only `rulesAgreedAt` into the auth-store account.
- **Inputs:** `chapters` — ordered server-rendered `RulesDocument` elements (one per `RULES_CHAPTER_IDS` id).
- **Returns / side effects:** Heading, prompt, progress, current chapter, error alert, full-width **I agree** button, icon-only back after the first chapter. POSTs `/me/rules-agreement` via `agreeToRules` only on the last chapter. Renders `null` without a session or when `chapters` is empty.
