...
- **Purpose:** Fourth post-login screen after `account.setup` is null (name and address may be skipped; living-room rules agreement is required). Embeds `ForumLoader` (forum list + composer) below the heading; card is `max-w-xl`.
...
- **Used by:** `ForumLoader`, `MemberProfileScreen` (`composerHidden` for the profile note).

## Function: ContactLoader

- **Purpose:** Client loader for in-app contact on `/contact`. Session from `useAuthStore`; returns null without a session. Posts via `postContact`. Missing name or rules opens `RequirementsOverlay` (no Skip) before retry; a retried 409 sets `formError` instead of reopening the overlay. On success fetches conversations and navigates to `/messages` or `/messages?c=` for the official 21.gifts thread. No local success-hide of the form.
...
- **Used by:** `ForumLoader`, `ContactLoader`, `MemberProfileScreen`, `RequirementsOverlay` flow.
