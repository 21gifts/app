...
- **Purpose:** Remaining body text plus nested post cards for resolved `/messages/<uuid>` URLs. Fills from knownNotes first; otherwise fetchPublicMessage (catch, never throw). 404/null leaves the URL. Nested card is a link with name, role span pill, timestamp, optional photo blob, caption (no nested unfurl), ₿ amount. NoteTranslate on stripped display text only.
- **Inputs:** text, knownNotes, excludeId, rateDay, fiat, optional onActivate.
