...
- **Returns / side effects:** React tree; loads gift-day rates for forum fiat; lazily fetches the selected member posts or replies; fetches photos for displayed `hasPhoto` cards into blob URLs via `fetchMessagePhoto` and revokes them on unmount; empty or whitespace-only pay amount requests 21 sats; may `POST` invoice/conversation/replies and navigate to `/messages?c=` or a reply's `/messages/{parentId}`.
- **Used by:** `MemberProfileLoader`.
