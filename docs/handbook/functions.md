...
- **Purpose:** Stacks stills for a note with `photoCount > 1` in the page scrollport. There is no nested scroller. A `current/total` chip (`forum.galleryPosition`) sits on the stack. Dots (`gap-5`, `IconButton` `sm` ghost, `forum.galleryDot`) move that one scrollport to the still. Empty `photos` returns `null`. Single still has the chip and no dots. The capture time is stored, not shown.
