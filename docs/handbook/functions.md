...
- **Purpose:** Login UI: one **Log in** button (existing login, or create when the browser has none), then signed-in steps: `NameForm` if no name, `LightningAddressForm` if no address, then a welcome with a gift icon and **Send a gift**. Visitor-facing copy via `useTranslations`.
