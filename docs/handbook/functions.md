...
- **Inputs:** Reads `useAuthStore`. User input: address string. Visitor-facing copy via `useTranslations`. Empty, example, not-found, and request failures are typed keys so they re-render after a locale change.
