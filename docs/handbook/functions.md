...
- **Purpose:** Profile identity-card Notifications section: uppercase heading `profile.push.heading`, a `SegmentedControl tone="neutral"` All / Active / Mentions (`profile.push.level.all` / `active` / `mentions`, group `profile.push.level.label`) whenever a session exists, a muted hint (`profile.push.level.hint`), and an icon-only Bell `IconButton` as the control's trailing slot in the same pill when `serviceWorker` / `PushManager` are ready. Off is `variant="secondary"` with an outline BellOff; on is `variant="primary"` with a filled Bell (`fill="currentColor"`). The button stays icon-only — fill vs outline plus `aria-pressed` encode on/off, not visible On/Off copy. Changing the level POSTs `/me/notification-level` via `postNotificationLevel` and merges `notificationLevel` into the current store account when the session still matches; a second change while the POST is in flight is ignored; failure keeps the previous value and shows `profile.push.level.error`. Renders nothing without a session. On iPhone Safari outside standalone, shows `profile.push.installHint` under the level hint.
...
- **Returns / side effects:** Heading, All/Active/Mentions control (icon-only bell in the same `SegmentedControl` pill when Push APIs are ready), muted hint, and (when Push APIs are ready) icon-only `IconButton` named from `profile.push.enable` or `profile.push.disable` (`aria-pressed` when subscribed). Level change calls `postNotificationLevel` then merges `notificationLevel`; user gesture on the bell calls enable/disable; may show `profile.push.unavailable` on push failure or `profile.push.level.error` on level failure.
- **Used by:** `ProfileScreen`.
...
- **Inputs:** `value`, `options` (`value` + `label`, optional `badge` / `badgeAriaLabel`; chip omitted when `badge` is missing or ≤ 0), `onChange`, `ariaLabel`, `tone`, optional `shell` (default `app`, gift only), optional `className` (on the option track), optional `trailing` (neutral only).
- **Returns / side effects:** A `role="group"` track of `type="button"` options with `aria-pressed`. Neutral without `trailing` is that group as the pill (`className` lands there, including Forum `!grid`). Neutral with `trailing` wraps the group in a non-group pill plus a 1px separator and the trailing node. Gift ignores `trailing`. No network.
...
- **Used by:** `StatsLoader`, `ForumLoader`, `PublicMessageLoader`, `MemberProfileScreen`, `ModerateScreen`.
