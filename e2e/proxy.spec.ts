...
test('same-origin api proxy routes exist', async ({ request }) => {
  ...
  expect((await request.post('/me/lightning-address/verification')).status()).toBe(502);
  expect((await request.post('/me/lightning-address/verification/confirm')).status()).toBe(502);
