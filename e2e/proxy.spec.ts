...
test('same-origin api proxy routes exist', async ({ request }) => {
  ...
  expect((await request.get('/gifts/stats')).status()).toBe(404);
  expect((await request.post('/auth/passkey/register/begin')).status()).toBe(404);
  expect((await request.post('/auth/passkey/register/finish')).status()).toBe(404);
  expect((await request.post('/auth/passkey/authenticate/begin')).status()).toBe(404);
  expect((await request.post('/auth/passkey/authenticate/finish')).status()).toBe(404);
});
